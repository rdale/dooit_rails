class TaggedVtodosController < ApplicationController
  ...
  def index
   ...
   @vtodos = tag.vtodos
  @stat = tag
  #needs to show only active and completed for certain tag
   @vtodo_cs = tag.vtodos.find(:all, :conditions => "status = 'done'")
   @vtodo_as = tag.vtodos.find(:all, :conditions => "status = ''")
   @tags = Tag.find(:all)
